...
c----------------------------------------------------------------------------c
      ...
      open(unit=16,file='rilm_gauss.dat',status='unknown')
      open(unit=17,file='config_gauss.dat',status='unknown')
      open(unit=18,file='trajectory.dat',status='unknown')
      open(unit=19,file='qmdist.dat',status='unknown')
      open(unit=20,file='rcor_gauss.dat',status='unknown')
      open(unit=21,file='rcor2_gauss.dat',status='unknown')
      open(unit=22,file='rcor3_gauss.dat',status='unknown') 
      ...
      write(6,*) 'number of heating sweeps (nheat=5)'
      read(5,*)  nheat
      write(6,*) 'update x (delx=0.5)'
      read(5,*)  delx
...
c----------------------------------------------------------------------------c
      ...
      write(16,*)   'qm rilm gauss 1.0'
      write(16,*)   '-----------------'
      write(16,101) f,n,a
      write(16,102) nin,nmc
      write(16,103) np,nc
      write(16,104) nheat,delx
      write(16,*)
      write(17,*)   '#',n,nmc/kp,n*a,f
      
 101  format(1x,' f    = ',f8.2,' n   = ',i5,' a   = ',f5.4)
 102  format(1x,' nin  = ',i8  ,' nmc = ',i5)
 103  format(1x,' np   = ',i8  ,' nc  = ',i5)
 104  format(1x,' nheat= ',i8  ,' delx= ',f5.4)
...
c----------------------------------------------------------------------------c
c     distribution of instantons                                             c
c----------------------------------------------------------------------------c
...
c----------------------------------------------------------------------------c
 ...
 150     continue   
         ...
         if (mod(i,kp) .eq. 0) then
            ...
            write(6,*) 'acceptance rate ',float(nacc)/(float(nhit)+0.01)
         endif
...
c---------------------------------------------------------------------------c
            ...
            xmin = abs(f*tanh(f*a))
            if(abs(x(j)) .lt. xmin) goto 860

c           if(abs(x(j-1)) .lt. xmin) then 
c              vel  = (x_hot(j)-x_hot(j-1))/(2.0*a)
c              sjak =-log(abs(vel))
c              sold = sold + sjak
c           else if(abs(x(j+1)) .lt. xmin) then 
c              vel  = (x_hot(j+1)-x_hot(j))/(2.0*a)
c              sjak =-log(abs(vel))
c              sold = sold + sjak
c           endif
...
c---------------------------------------------------------------------------c
            ...
            xnew = x_hot(j) + delx*(2.0*ranp(iseed2)-1.0)
...
c           if(abs(x(j-1)) .lt. xmin) then 
c              vel  = (xnew-x_hot(j-1))/(2.0*a)
c              sjak =-log(abs(vel))
c              sold = sold + sjak
c           else if(abs(x(j+1)) .lt. xmin) then 
c              vel  = (x_hot(j+1)-xnew)/(2.0*a)
c              sjak =-log(abs(vel))
c              sold = sold + sjak
c           endif
...
c----------------------------------------------------------------------------c
         ...
         if (mod(i,kp) .eq. 0) then
            write(17,*) '# configuration',i
            do 155 k=1,n
               write(17,*) k*a,x_hot(k)
 155        continue
         endif
...
c----------------------------------------------------------------------------c
         ...
         do 160 k=1,n
            call lens(x_hot(k),xhist_min,stxhist,nxhist,ix)
            x_sum = x_sum + x_hot(k)
            x2_sum= x2_sum+ x_hot(k)**2
            x4_sum= x4_sum+ x_hot(k)**4
            x8_sum= x8_sum+ x_hot(k)**8
 160     continue
...
c----------------------------------------------------------------------------c
      ...
      write(16,907) tv_av,tv_err
      write(16,908) e_av,e_err
      write(16,909) x_av,x_err
